...
import type { GcpProject } from '@/shared/infra/contracts/i-cloud-provider.ts';
import { generateSshKeyPair } from '@/shared/infra/ssh/ssh-utils.ts';
    ...
    <Box flexDirection="column" gap={1}>
      ...
      <Box paddingX={2} flexDirection="column" flexGrow={1}>
        ...
        {step === 2 && <AuthProject 
          ...
          onVerifyAuth={async () => cloudProvider?.verifyAuth() ?? false}
          ...
          onEnableApis={async (projectId) => {
            await cloudProvider?.enableApis(projectId);
          }}
        />}
        {step === 3 && <RegionSelect 
          wizardStore={wizardStore} 
